...
function renderAudit() {
  ...
  $('#auditTable tbody').innerHTML = rows.map(a =>
    `<tr><td>${esc(a.action)}</td><td>${esc(a.user)}</td><td>${new Date(a.timestamp).toLocaleString('ar-EG')}</td><td>${esc(a.bookingId)}</td><td>${esc(a.details||'')}</td></tr>`
  ).join('') || '<tr><td colspan="5" style="text-align:center;color:#888">لا توجد سجلات</td></tr>';
}
...
document.addEventListener('click', e => {
  ...
  if (t === 'schedule') {
    ...
    exportSheet(auditLog.map(a=>({'الإجراء':a.action,'المستخدم':a.user,'التاريخ':a.timestamp,'معرف الحجز':a.bookingId,'التفاصيل':a.details||''})), 'audit');
