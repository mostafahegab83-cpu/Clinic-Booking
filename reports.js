...
function renderAudit() {
  ...
  $('#auditTable tbody').innerHTML = rows.map(a => {
    const info = bookingInfo(a.bookingId);
...
document.addEventListener('click', e => {
  ...
  if (t === 'schedule') {
    ...
    exportSheet(auditLog.map(a=>{const i=bookingInfo(a.bookingId);return{'الإجراء':a.action,'المستخدم':a.user,'التاريخ':a.timestamp,'اسم العميل':i.name,'وسيلة الاتصال':i.contact,'التفاصيل':a.details||''};}), 'audit');
