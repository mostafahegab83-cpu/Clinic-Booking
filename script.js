// ============ State ============
const STORAGE_KEY = 'clinic_bookings_v1';
const SPECIALTIES_KEY = 'clinic_specialties_v1';
let bookings = JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]');
let customSpecialties = JSON.parse(localStorage.getItem(SPECIALTIES_KEY) || '[]');
let currentStep = 1;
let editingId = null;
const totalSteps = 5;


// ============ Elements ============
const $ = (sel) => document.querySelector(sel);
const $$ = (sel) => document.querySelectorAll(sel);

const stepButtons = $$('.step');
const panels = $$('.step-panel');
const prevBtn = $('#prevBtn');
const nextBtn = $('#nextBtn');
const saveBtn = $('#saveBtn');

// ============ Step Navigation ============
function showStep(n) {
  currentStep = n;
  stepButtons.forEach((b, i) => {
    b.classList.toggle('active', i + 1 === n);
    b.classList.toggle('completed', i + 1 < n);
  });
  panels.forEach((p) => p.classList.toggle('active', +p.dataset.panel === n));
  prevBtn.style.display = n === 1 ? 'none' : '';
  nextBtn.style.display = n === totalSteps ? 'none' : '';
  saveBtn.style.display = n === totalSteps ? '' : 'none';
  updateSummary();
}

stepButtons.forEach((b) => b.addEventListener('click', () => showStep(+b.dataset.step)));
prevBtn.addEventListener('click', () => currentStep > 1 && showStep(currentStep - 1));
nextBtn.addEventListener('click', () => {
  if (!validateStep(currentStep)) return;
  if (currentStep < totalSteps) showStep(currentStep + 1);
});

// ============ Validation ============
function validateStep(n) {
  const required = {
    1: ['#fullName', '#idNumber', '#birthDate', '#phone', '#gender'],
    2: ['#specialty'],
    3: ['#appointmentDate', '#doctor'],
    4: ['#appointmentTime', '#duration'],
    5: [],
  };
  for (const sel of required[n] || []) {
    const el = $(sel);
    if (!el.value.trim()) {
      el.focus();
      toast('يرجى تعبئة الحقول المطلوبة');
      return false;
    }
  }
  if (n === 2 && $('#specialty').value === '__custom__' && !$('#specialtyCustom').value.trim()) {
    $('#specialtyCustom').focus();
    toast('يرجى إدخال اسم التخصص');
    return false;
  }
  return true;
}

// ============ Specialties (custom) ============
const specialtySelect = $('#specialty');
const specialtyCustom = $('#specialtyCustom');

function injectCustomSpecialties() {
  const customOpt = specialtySelect.querySelector('option[value="__custom__"]');
  customSpecialties.forEach(name => {
    if (![...specialtySelect.options].some(o => o.value === name || o.text === name)) {
      const opt = document.createElement('option');
      opt.textContent = name;
      specialtySelect.insertBefore(opt, customOpt);
    }
  });
}
injectCustomSpecialties();

specialtySelect.addEventListener('change', () => {
  if (specialtySelect.value === '__custom__') {
    specialtyCustom.style.display = '';
    specialtyCustom.focus();
  } else {
    specialtyCustom.style.display = 'none';
    specialtyCustom.value = '';
  }
});

specialtyCustom.addEventListener('blur', () => {
  const name = specialtyCustom.value.trim();
  if (!name) return;
  if (![...specialtySelect.options].some(o => o.text === name)) {
    const opt = document.createElement('option');
    opt.textContent = name;
    specialtySelect.insertBefore(opt, specialtySelect.querySelector('option[value="__custom__"]'));
    customSpecialties.push(name);
    localStorage.setItem(SPECIALTIES_KEY, JSON.stringify(customSpecialties));
  }
  specialtySelect.value = name;
  specialtyCustom.style.display = 'none';
  specialtyCustom.value = '';
  updateSummary();
});

function getSpecialtyValue() {
  if (specialtySelect.value === '__custom__') return specialtyCustom.value.trim();
  return specialtySelect.value;
}

// ============ Form Data ============
function readForm() {
  return {
    id: editingId || Date.now().toString(),
    fullName: $('#fullName').value.trim(),
    idNumber: $('#idNumber').value.trim(),
    birthDate: $('#birthDate').value,
    phone: $('#phone').value.trim(),
    gender: $('#gender').value,
    email: $('#email').value.trim(),
    specialty: getSpecialtyValue(),
    condition: $('#condition').value.trim(),
    appointmentDate: $('#appointmentDate').value,
    doctor: $('#doctor').value,
    appointmentTime: $('#appointmentTime').value,
    duration: $('#duration').value,
    notes: $('#notes').value.trim(),
  };
}


function fillForm(b) {
  $('#fullName').value = b.fullName || '';
  $('#idNumber').value = b.idNumber || '';
  $('#birthDate').value = b.birthDate || '';
  $('#phone').value = b.phone || '';
  $('#gender').value = b.gender || '';
  $('#email').value = b.email || '';
  if (b.specialty && ![...specialtySelect.options].some(o => o.text === b.specialty)) {
    const opt = document.createElement('option');
    opt.textContent = b.specialty;
    specialtySelect.insertBefore(opt, specialtySelect.querySelector('option[value="__custom__"]'));
  }
  $('#specialty').value = b.specialty || '';
  $('#specialtyCustom').style.display = 'none';
  $('#specialtyCustom').value = '';
  $('#condition').value = b.condition || '';
  $('#appointmentDate').value = b.appointmentDate || '';
  $('#doctor').value = b.doctor || '';
  $('#appointmentTime').value = b.appointmentTime || '';
  $('#duration').value = b.duration || '30';
  $('#notes').value = b.notes || '';
}

function clearForm() {
  ['#fullName','#idNumber','#birthDate','#phone','#gender','#email',
   '#specialty','#condition','#appointmentDate','#doctor',
   '#appointmentTime','#notes'].forEach(s => $(s).value = '');
  $('#duration').value = '30';
  editingId = null;
}

// ============ Summary ============
function updateSummary() {
  const d = readForm();
  $('#sName').textContent = d.fullName || '-';
  $('#sCondition').textContent = d.specialty || '-';
  $('#sDate').textContent = d.appointmentDate || '-';
  $('#sTime').textContent = d.appointmentTime ? `${d.appointmentTime} / ${d.duration} د` : '-';
  $('#sDoctor').textContent = d.doctor || '-';
  const note = $('#summaryNote');
  if (d.appointmentDate && d.appointmentTime) {
    note.textContent = `📅 ${d.appointmentDate} - ${d.appointmentTime}`;
  } else {
    note.textContent = '📅 لم يتم اختيار موعد بعد';
  }
}
document.addEventListener('input', updateSummary);
document.addEventListener('change', updateSummary);

// ============ Save / Edit / Delete ============
saveBtn.addEventListener('click', () => {
  for (let i = 1; i <= totalSteps; i++) if (!validateStep(i)) { showStep(i); return; }
  const data = readForm();
  if (editingId) {
    bookings = bookings.map(b => b.id === editingId ? data : b);
    toast('تم تحديث الحجز');
  } else {
    bookings.push(data);
    toast('تم حفظ الحجز بنجاح');
  }
  localStorage.setItem(STORAGE_KEY, JSON.stringify(bookings));
  clearForm();
  showStep(1);
  renderTable();
});

function editBooking(id) {
  const b = bookings.find(x => x.id === id);
  if (!b) return;
  editingId = id;
  fillForm(b);
  showStep(1);
  window.scrollTo({ top: 0, behavior: 'smooth' });
  toast('وضع التعديل - عدّل البيانات ثم احفظ');
}

function deleteBooking(id) {
  if (!confirm('هل أنت متأكد من حذف هذا الحجز؟')) return;
  bookings = bookings.filter(b => b.id !== id);
  localStorage.setItem(STORAGE_KEY, JSON.stringify(bookings));
  renderTable();
  toast('تم حذف الحجز');
}

// ============ Table ============
function renderTable(filter = '') {
  const tbody = $('#bookingsTable tbody');
  const empty = $('#emptyMsg');
  const f = filter.toLowerCase();
  const rows = bookings.filter(b =>
    !f || b.fullName.toLowerCase().includes(f) || b.phone.includes(f)
  );
  tbody.innerHTML = rows.map(b => `
    <tr>
      <td>${escape(b.fullName)}</td>
      <td>${escape(b.phone)}</td>
      <td>${escape(b.doctor)}</td>
      <td>${escape(b.appointmentDate)}</td>
      <td>${escape(b.appointmentTime)}</td>
      <td class="row-actions">
        <button class="btn small ghost" onclick="editBooking('${b.id}')">✏️ تعديل</button>
        <button class="btn small danger" onclick="deleteBooking('${b.id}')">🗑️ حذف</button>
      </td>
    </tr>
  `).join('');
  empty.style.display = rows.length ? 'none' : '';
  $('#bookingsTable').style.display = rows.length ? '' : 'none';
}

function escape(s) {
  return String(s ?? '').replace(/[&<>"']/g, c => ({
    '&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'
  }[c]));
}

$('#searchInput').addEventListener('input', (e) => renderTable(e.target.value));

// ============ Toast ============
let toastTimer;
function toast(msg) {
  let el = document.querySelector('.toast');
  if (!el) {
    el = document.createElement('div');
    el.className = 'toast';
    document.body.appendChild(el);
  }
  el.textContent = msg;
  el.classList.add('show');
  clearTimeout(toastTimer);
  toastTimer = setTimeout(() => el.classList.remove('show'), 2500);
}

// expose for inline handlers
window.editBooking = editBooking;
window.deleteBooking = deleteBooking;

// ============ Init ============
showStep(1);
renderTable();
